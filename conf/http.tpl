<!-- rclone serve http template for github.com/opgcn/dsftp-client -->
...
/*	.hideable {
	...
	<body onload='filter();toggle("order");changeSize()'>
		...
		<header>
			<h4>
				DSFTP_PREFIX{{range $i, $crumb := .Breadcrumb}}<a href="{{html $crumb.Link}}">{{html $crumb.Text}}</a>{{if ne $i 0}}/{{end}}{{end}} 
			</h4>
		</header>
		<main>
			<div class="meta">
				<div id="summary">
					<span class="meta-item"><input type="text" placeholder="过滤条件" id="filter" onkeyup='filter()'></span>
				</div>
			</div>
			<div class="listing">
				<table aria-describedby="summary">
					...
					<tr>
						...
						<th>
							...
							<a href="?sort=name&order=asc" class="order">名称</a>
						</th>
						<th>
							<a href="?sort=size&order=asc" class="order">大小</a>
						</th>
						<th class="hideable">
							<a href="?sort=time&order=asc" class="order">修改时间</a>
						</th>
        ...
        <footer>
			<h6>
				此页面由 <a href="https://github.com/opgcn/dsftp-client" target="_blank">dsftp-client</a> 生成
			</h6>
		</footer>
		<script>
